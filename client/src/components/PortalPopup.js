...
const PortalPopup = ({
  ...
  const memoizedRelativeLayerRef = relativeLayerRef?.current;
  ...
  const popupStyle = useMemo(() => {
    ...
    style.opacity = 1;
    return style;
  }, [placement, overlayColor, zIndex, memoizedRelativeLayerRef]);

  const setPosition = useCallback(() => {
    const currentRelativeLayerRef = relativeLayerRef?.current;
    const relativeItem = currentRelativeLayerRef?.getBoundingClientRect();
    ...
    bottom,
    placement,
    relativeLayerRef, // Add relativeLayerRef here
